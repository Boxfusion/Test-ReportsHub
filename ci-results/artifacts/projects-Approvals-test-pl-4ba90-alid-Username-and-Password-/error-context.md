...
- Name: projects/Approvals/test-plans/Login/invalid-login.spec.ts >> TC-01 — Invalid (Username and Password)
- Location: projects/Approvals/test-plans/Login/invalid-login.spec.ts:11:5
...
```ts
  1  | // AUTO-RECORDED from test-plans/Login/invalid-login.md
  2  | // Source: Azure DevOps test plan #100853, suite #100854, test case #104705
  ...
  9  | const INVALID_USER = { username: 'invaliduser', password: 'wrongpass123' };
  10 | 
  11 | test('TC-01 — Invalid (Username and Password)', async ({ page }) => {
...
> 17 |   await expect(page.getByPlaceholder(/username/i)).toBeVisible();
  ...
  20 | 
  21 |   // STEP 3: TYPE Username field with `invaliduser`
  22 |   await page.getByPlaceholder(/username/i).fill(INVALID_USER.username);
  23 | 
  24 |   // STEP 4: TYPE Password field with `wrongpass123`
  25 |   await page.getByPlaceholder(/password/i).fill(INVALID_USER.password);
  26 | 
  27 |   // ASSERT Username/Password fields accept the entered invalid values
  28 |   await expect(page.getByPlaceholder(/username/i)).toHaveValue(INVALID_USER.username);
  29 |   await expect(page.getByPlaceholder(/password/i)).toHaveValue(INVALID_USER.password);
  30 | 
