...
> ```
> node scripts/run-plan.js projects/HCM/test-plans/<folder>/<plan>.md
> node scripts/build-project-dashboard.js --project=HCM
> ```
...
## 4. Report Format
Reports saved to `test-reports/YYYY-MM-DD/<plan-name>.md` (relative to the project root, i.e. `projects/HCM/test-reports/...`):
```
# Report: <Plan Title>
...
node scripts/build-project-dashboard.js --project=HCM
```
The dashboard (`projects/HCM/index.html`) is auto-generated from every plan in this project's `test-plans/` and every report under `test-reports/`. **Never hand-edit it.**

## 7. Allure Report Generation
After every test run, regenerate the per-project Allure report (from hub root):
```bash
rm -rf projects/HCM/allure-results
node scripts/generate-allure-results.js --project=HCM
npx allure generate projects/HCM/allure-results --clean -o projects/HCM/allure-report
```
...
### Execution flow
1. `node scripts/run-plan.js projects/HCM/test-plans/<folder>/<plan>.md` — runs Playwright, emits a JSON summary.
