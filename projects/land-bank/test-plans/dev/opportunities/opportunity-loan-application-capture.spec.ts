...
function credsFor(role: string) {
  ...
  // Environment-namespaced credentials win: TEST_ENV=phase2 → PHASE2_RM_USERNAME.
  // Falls back to the bare <ROLE>_* keys, which the other hub projects still use.
  // Without the prefix every environment collapses onto whichever block .env lists
  // first (playwright.config.ts keeps the FIRST definition of a key), which silently
  // ran QA and Phase2 with DEV logins.
