# CLAUDE.md — Hybrid Markdown + Playwright Testing (PD-Dispatch project)

> **Multi-project hub.** This file describes the **PD-Dispatch** project. Shared Playwright + Allure infrastructure (`package.json`, `playwright.config.ts`, `node_modules/`, `scripts/run-plan.js`) lives at the hub root one level up. Tests run from the hub root: `node scripts/run-plan.js projects/PD-Dispatch/test-plans/<folder>/<plan>.md`.

> **Relationship to NC Dispatch.** PD-Dispatch is the Boxfusion **master / product-development site** for the Dispatcher Admin Portal — it carries **more functionality** than the client-facing [NC Dispatch](../dispatch/CLAUDE.md) (Northern Cape) site. Everything is tested here on PD **first**, then promoted to the client site. Both share the same ADO project (`pd-dispatcher-V2`); PD-Dispatch is driven by its own (still-incomplete) test plan #104631.
...
2. `/RunTest` runs Playwright first: `node ../../scripts/run-plan.js projects/PD-Dispatch/test-plans/<folder>/<name>.md` (from hub root).
...
6. Regenerate the project dashboard (the hub's `scripts/build-project-dashboard.js --project=PD-Dispatch`) and the per-project Allure report.
...
1. Read this file (`projects/PD-Dispatch/CLAUDE.md`) completely
...
| URL | https://pd-dispatcher-v2-adminportal-qa.shesha.app/login |
| Environment | QA |
...
| Project | pd-dispatcher-V2 |
| Test Plan | [#104631](https://dev.azure.com/boxfusion/pd-dispatcher-V2/_testPlans/define?planId=104631&suiteId=104635) |
| Test Suite | #104635 |

> **Note:** The PD-Dispatch test plan (#104631) is **not yet complete** on DevOps — expect gaps vs. the eventual full suite.
