...
> ```
> node scripts/run-plan.js projects/PD-Dispatch/test-plans/<folder>/<plan>.md
> node scripts/build-project-dashboard.js --project=PD-Dispatch
> ```
...
## 4. Report Format
Reports saved to `test-reports/YYYY-MM-DD/<plan-name>.md` (relative to the project root, i.e. `projects/PD-Dispatch/test-reports/...`):
```
# Report: <Plan Title>
...
node scripts/build-project-dashboard.js --project=PD-Dispatch
```
The dashboard (`projects/PD-Dispatch/index.html`) is auto-generated from every plan in this project's `test-plans/` and every report under `test-reports/`. **Never hand-edit it.**

## 7. Allure Report Generation
The runner (`scripts/run-plan.js`) generates the per-project Allure report automatically after a run, into `projects/PD-Dispatch/allure-report/` (or `allure-report--<label>/` for a labelled variant run). To regenerate manually from the hub root:
```bash
npx allure generate projects/PD-Dispatch/allure-results --clean --single-file -o projects/PD-Dispatch/allure-report
```
...
### Execution flow
1. `node scripts/run-plan.js projects/PD-Dispatch/test-plans/<folder>/<plan>.md` — runs Playwright, emits a JSON summary.
