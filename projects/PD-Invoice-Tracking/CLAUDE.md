...
| URL | https://pd-invoicetracking-adminportal-qa.shesha.app/login |
| Environment | QA |

## Credentials

> ⚠️ **The app moved to Shesha hosting on 2026-09-14** — `https://pd-invoicetracking-adminportal-qa.shesha.app/login`.
> The workflow changed with it, and the step→actor map below **supersedes** the ThulileM/TaniaSmith set
> used up to 2026-09-13. Those older accounts are **unverified on the new host** — do not assume they work.
> All passwords: `123qwe`.

### Step → actor (new Shesha app, supplied by the user 2026-09-14)
| Workflow step | Username |
|---|---|
...
> **Actor ≠ nominee.** As established on the old host, a step's "role" label names the person being
> *assigned* on the form, not who actions it. Use the table above for the login.

### Legacy accounts (old azurewebsites host — unverified on Shesha)
