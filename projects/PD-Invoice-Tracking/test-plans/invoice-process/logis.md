...
> **App:** Invoice Tracking (ITS) Admin Portal — https://pd-invoicetracking-adminportal-qa.shesha.app/login (QA)
> **Logins (Shesha app, 2026-09-14):** Register/Certify/Verify = **JohanV**; Approve/Assign Official/Capture & Link/Pre-Authorise/Verify Voucher = **FatimaP**; BAS + Stub imports = **Admin**; Capture Filing = **ThabisoM**. All passwords `123qwe`. The old ThulileM/TaniaSmith/GwenB/MoshadiM accounts do NOT exist here.
...
3. TYPE Username field with `JohanV`
...
3. CLICK Create New button
   - ASSERT the process list shows DBE LOGIS Request For Payment / LOGIS Request For Payment / Order Uploading / Request For Payment (BAS is NOT offered on the Shesha app — not developed yet, confirmed 2026-09-14)
4. SNAPSHOT — locate "LOGIS Request For Payment"
5. CLICK **LOGIS Request For Payment** (match EXACTLY — "DBE LOGIS Request For Payment" is a different process sitting directly above it)
...
9. CLICK an order from the list
   - ASSERT the order populates Order No, and auto-fills Supplier Name (read-only), Email, Order Description
   - **Business Unit** is REQUIRED. It auto-fills only when the order’s End User maps to a portal user; otherwise it stays blank. Submit remains DISABLED until it is set.
   - ⚠️ Wait for the invoice attachment to finish uploading (its rendered file SIZE appears) before clicking Add — clicking while the upload is in flight fails with "Create failed" and silently loses the row. See `test-reports/bugs/2026-09-14-logis-add-invoice-line-during-upload-fails-silently.md`
   - Whether OVERRIDING a pre-filled Business Unit is itself a problem is untested — the spec skips orders that pre-fill it.
...
20. CLICK the Add icon
    - ASSERT the invoice row is added and the Total Amount reflects the invoice amount
21. *(This build has NO Order Line Item panel — steps 21-22 describe an older build and do not apply; verified 2026-09-14.)*
22. ASSERT the Submit button is enabled (it enables once Business Unit is set)
...
25. CLICK Submit button
    - *(No "Submit Invoice with Order Line Items" dialog appears on this build — there are no Order Line Items. The spec confirms one only if present.)*
26. (n/a on this build)
27. (n/a on this build)
    - ASSERT (BLOCKING) the system redirects and the item leaves the register step, routed to **Certify Invoice** (JohanV). ("Match to Order and Verify" belongs to the separate DBE LOGIS process, not this one.)
...
**Role:** Filing — **ThabisoM** (reassigned 2026-09-14). GwenB / Gwen Simbeni was the OLD app's filing role and does not exist on the Shesha tenant; before the reassignment the step sat on `aakil`, whose password is unknown. **Precondition:** item at Capture Filing (status Paid). **Estimated duration:** 45s
1. CLICK Invoice Attachment link → ASSERT downloads
2. TYPE the Batch Number — on this build it is an **editable required field** (marked *), NOT pre-populated read-only as on the old app (verified 2026-09-14)
