# CLAUDE.md — Hybrid Markdown + Playwright Testing (NC Dispatch project)

> **Multi-project hub.** This file describes the **NC Dispatch** project. Shared Playwright + Allure infrastructure (`package.json`, `playwright.config.ts`, `node_modules/`, `scripts/run-plan.js`) lives at the hub root one level up. Tests run from the hub root: `node scripts/run-plan.js projects/dispatch/test-plans/<folder>/<plan>.md`.
...
2. `/RunTest` runs Playwright first: `node ../../scripts/run-plan.js projects/dispatch/test-plans/<folder>/<name>.md` (from hub root).
...
6. Regenerate the project dashboard (the hub's `scripts/build-project-dashboard.js --project=dispatch`) and the per-project Allure report.
...
1. Read this file (`projects/dispatch/CLAUDE.md`) completely
...
| Test Plan | [#65099](https://dev.azure.com/boxfusion/pd-dispatcher-V2/_testPlans/define?planId=65099&suiteId=65100) |
| Test Suite | #65100 |
