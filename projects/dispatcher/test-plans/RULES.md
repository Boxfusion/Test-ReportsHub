...
> ```
> node scripts/run-plan.js projects/dispatcher/test-plans/<folder>/<plan>.md
> node scripts/build-project-dashboard.js --project=dispatcher
> ```
...
## 4. Report Format
Reports saved to `test-reports/YYYY-MM-DD/<plan-name>.md` (relative to the project root, i.e. `projects/dispatcher/test-reports/...`):
```
# Report: <Plan Title>
...
node scripts/build-project-dashboard.js --project=dispatcher
```
The dashboard (`projects/dispatcher/index.html`) is auto-generated from every plan in this project's `test-plans/` and every report under `test-reports/`. **Never hand-edit it.**

## 7. Allure Report Generation
After every test run, regenerate the per-project Allure report (from hub root):
```bash
rm -rf projects/dispatcher/allure-results
node scripts/generate-allure-results.js --project=dispatcher
npx allure generate projects/dispatcher/allure-results --clean -o projects/dispatcher/allure-report
```
...
### Execution flow
1. `node scripts/run-plan.js projects/dispatcher/test-plans/<folder>/<plan>.md` — runs Playwright, emits a JSON summary.
