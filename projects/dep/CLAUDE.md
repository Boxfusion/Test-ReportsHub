# CLAUDE.md — Hybrid Markdown + Playwright Testing (dep project)

> **Multi-project hub.** This file describes the **dep** project. Shared Playwright + Allure infrastructure (`package.json`, `playwright.config.ts`, `node_modules/`, `scripts/run-plan.js`) lives at the hub root one level up. Tests run from the hub root: `node scripts/run-plan.js projects/dep/test-plans/<folder>/<plan>.md`.
...
2. `/RunTest` runs Playwright first: `node ../../scripts/run-plan.js projects/dep/test-plans/<folder>/<name>.md` (from hub root).
...
6. Regenerate the project dashboard (the hub's `scripts/build-project-dashboard.js --project=dep`) and the per-project Allure report.
...
1. Read this file (`projects/dep/CLAUDE.md`) completely
...
| Key | Value |
|-----|-------|
| App | Dep Admin Portal |
| URL | https://linux-dep-adminportal-test.azurewebsites.net/ |
| Environment | QA |
